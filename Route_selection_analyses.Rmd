...
write.csv(half1, paste0("HB1_allBirds_",Sys.Date(),".csv"), row.names = FALSE)
write.csv(half2, paste0("HB2_allBirds_",Sys.Date(),".csv"), row.names = FALSE)
```
...
Step-selection analyses use conditional logistic regressions to predict whether a location is used or unused based on environmental conditions. Once we have our files returned from Movebank's Env-DATA service, we can process the annotated data for modeling. Using the directions we calculated above, we can derive wind support from the wind data. Our second variable of interest is the vertical velocity of pressure, which proxies uplift and subsidence. Because pressure is lower with increasing altitude, negative vertical velocity values indicate uplift. Our third variable is the interaction between wind support and uplift because birds can select for both.

----
```{r import, results='hide', message=FALSE, warning=FALSE, eval=F}
# read in the annotated data
half1 <- read.csv(paste0(mypath,"HT_150s-4573417134914709132.csv"),
                  stringsAsFactors = F, header = T)
half2 <- read.csv(paste0(mypath,"HT_150s-4433398109146553161.csv"),
                  stringsAsFactors = F, header = T)

half1 <- read.csv(paste0(mypath, "annotations/HB_100s_1kNN_2022-04-29-3818566895793188085.csv"), stringsAsFactors = F, header = T)
half2 <- read.csv(paste0(mypath, "annotations/HB_100s_925NN_2022-04-29-8580129438169723627.csv"), stringsAsFactors = F, header = T)
pvvUV <- read.csv(paste0(mypath, "annotations/HB_100s_pvvUV_2022-04-29-4408269735548142621.csv"), stringsAsFactors = F, header = T) %>%
...
sst_blh <- read.csv(paste0(mypath, "annotations/HB_100s_blh_sst_NN_2022-04-29-7936675970549789811.csv"), stringsAsFactors = F, header = T)

colnames(half1)[16] <- "Temp_1000"
colnames(half2)[16] <- "Temp_925"
colnames(sst_blh)[16:18] <- c("BLH", "SST", "Temp_2m")

annotated_data <- half1
table(annotated_data$location.long == half2$location.long)
annotated_data$Temp_925 <- half2$Temp_925
annotated_data$u_wind <- pvvUV$u_wind
annotated_data$v_wind <- pvvUV$v_wind
annotated_data$tail <- wind_support(annotated_data$u_wind, annotated_data$v_wind, annotated_data$heading) 
annotated_data$BLH <- sst_blh$BLH
annotated_data$SST <- sst_blh$SST
annotated_data$Temp_2m <- sst_blh$Temp_2m
annotated_data <- annotated_data %>% 
  ...
  mutate(timestamp = as.POSIXct(strptime(timestamp, format = "%Y-%m-%d %H:%M:%S"),
         ...
         id_year = paste(id,year,sep="_"),
         case_ = as.numeric(case_), 
         deltaT = Temp_1000 - Temp_925, 
         ssDeltaT = SST - Temp_2m)
...
```{r rm_birds_silently, echo=F, warning=F, message=F, error=F, eval=F}
...
```{r readInvis, echo=F, warning=F, message=F, error=F}
save(annotated_data, file = "sst_pvv_blh_temp.RData")
annotated_data$delta <- annotated_data$Temp_1000 - annotated_data$Temp_925
df <- annotated_data %>% group_by(id_year,step_id_) %>% arrange(desc(delta), .by_group = T) %>% slice(1) %>% mutate(match = paste0(id_year_step, "_", lon2, "_", delta))# select the highest uplift per step


used <- annotated_data[which(annotated_data$case_ == 1),]
used$match <- paste0(used$id_year_step, "_", used$lon2, "_", used$delta)
used <- used[which(used$match %in% df$match),]

annotated_data$match <- paste0(annotated_data$id_year_step, "_", annotated_data$lon2, "_", annotated_data$delta)

full <- annotated_data[!(annotated_data$match %in% used$match),]
df <- df[!(df$match %in% used$match),]

ggplot(full, aes(Migration, delta))+geom_jitter(width = 0.35, alpha = 0.75, color = "black", shape = 1, data = df)+geom_jitter(data = full[which(full$case_ == 1),], alpha = 0.25, color = "red", width = 0.35)+geom_jitter(shape = 21, fill = "red", size = 2.5, width=0.35, data = used)+geom_boxplot(lwd = 0.75, alpha = 0)+labs(y = "Delta T")+theme_minimal()

df2 <- annotated_data[!is.na(annotated_data$delta),] %>% group_by(id_year,step_id_) %>% arrange(delta, .by_group = T) %>% slice(1) # select the lowest uplift per step
load(paste0(mypath, "annotated_data_25.1.21.RData"))
```
...
annotated_data <- annotated_data %>% 
  mutate(scaled_tail = as.numeric(scale(tail, center = T, scale = T)),
         scaled_deltaT = as.numeric(scale(deltaT, center = T, scale = T)),
...
# isolate strata containing values for SST
annotated_data$stratum <- paste0(annotated_data$id_year, annotated_data$step_id_)

seacrossing_points <- !is.na(annotated_data$SST)
sum(seacrossing_points) # 99,489 NA values
# select sea crossings
seacrossings <- annotated_data[seacrossing_points == T,]

# remove them from the data to remove sea-crossings
annotated_data <- annotated_data %>%
  filter(!stratum %in% seacrossings$stratum)
...
We need to model each individual in each year separately, which means fitting as many models as there are tracks. We can do this quickly using nested lists if we first write our model as a function and then pass it to the "purrr" package's "map" function.
...
# fit a single model to all the data including age as a predictor
annotated_data$Migration <- as.factor(annotated_data$Migration)
model_TVdT <- function(df) {
  clogit(case_ ~ scaled_tail + scaled_deltaT + scaled_step + scaled_angle + scaled_tail*scaled_deltaT + Migration +
           strata(id_year_step), data = df)
}

model_TVdT3 <- function(df) {
  clogit(case_ ~ scaled_tail + Migration + scaled_deltaT + scaled_step + scaled_angle + scaled_tail*scaled_deltaT*Migration + 
           strata(id_year_step), data = df)
}
...
SSF_results <- annotated_data %>% 
  group_by(id_year) %>% 
  nest() %>% 
  mutate(ssf_modelTU = purrr::map(data, modelTU),
         ssf_coefsTU = purrr::map(ssf_modelTU, coef),
         #ssf_se = purrr::map(coef(summary(ssf_modelTU))[, "se(coef)"]))
         ssf_summary = purrr::map(ssf_modelTU, summary),
         ssf_seT = purrr::map(sqrt(diag(vcov(ssf_summary)))[1]))
...
# plot_data <- unnest(SSF_results, ssf_coefsTU)
# # label the coefficients by variable
# plot_data$var <- rep(c("Wind support", "Boundary layer height", "ln_sl", "cos_ta", "Interaction"), times = nrow(plot_data)/5)
```
...
SSF_results$stage <- NA # create an empty column to store values
SSF_results$stage[which(SSF_results$id == "Annika" |
                         SSF_results$id == "Jouko" |
                         SSF_results$id == "Mikko" |
                         SSF_results$id == "Paivi" |
                         SSF_results$id == "Tiina" )] <- "adult" # label individuals of unknown age
SSF_results$stage[which(is.na(SSF_results$stage))] <- "juvenile" # label individuals tagged as fledglings

#add the number of each migration
SSF_results$Migration <- NA 
SSF_results$Migration[which(SSF_results$id_year == "Jaana_2013" | # add year 2 info
                             SSF_results$id_year == "Lars_2013" | 
                             SSF_results$id_year == "Mohammed_2018" | 
                             SSF_results$id_year == "Senta_2015" | 
                             SSF_results$id_year == "Valentin_2015" |
                             SSF_results$id_year == "Annika_2012" | 
                             SSF_results$id_year == "Jouko_2012" |
                             SSF_results$id_year == "Paivi_2014" |
                             SSF_results$id_year == "Mikko_2012" |
                             SSF_results$id_year == "Tiina_2012")] <- "2"

SSF_results$Migration[which(SSF_results$id_year == "Jaana_2014" | # add year 3 info
                             SSF_results$id_year == "Lars_2014" | 
                             SSF_results$id_year == "Senta_2016" |
                             SSF_results$id_year == "Annika_2013" |
                             SSF_results$id_year == "Jouko_2013" |
                             SSF_results$id_year == "Mikko_2013" |
                             SSF_results$id_year == "Paivi_2015" |
                             SSF_results$id_year == "Mohammed_2019" |
                             SSF_results$id_year == "Tiina_2013")] <- "3"

SSF_results$Migration[which(SSF_results$id_year == "Lars_2015" | # add year 4 info
                             SSF_results$id_year == "Mohammed_2020")] <- "4"

SSF_results$Migration[which(SSF_results$id_year == "Annika_2014" | # add year 5 info
                             SSF_results$id_year == "Jouko_2014" |
                             SSF_results$id_year == "Mikko_2014" | 
                             SSF_results$id_year == "Mikko_2015" |
                             SSF_results$id_year == "Paivi_2016" |
                             SSF_results$id_year == "Tiina_2014" |
                             SSF_results$id_year == "Tiina_2015")] <- "5 +" 
SSF_results$Migration[which(is.na(SSF_results$Migration))] <- "1" # everything else is year 1

SSF_results$category <- NA
SSF_results$category[which(SSF_results$Migration == 1)] <- "First year"
SSF_results$category[which(SSF_results$Migration == "5 +")] <- "Adult"
SSF_results$category[which(SSF_results$Migration %in% c(2,3,4))] <- "Juvenile"
SSF_results$category <- factor(SSF_results$category, levels = c("First year", "Juvenile", "Adult"))
...
SSF_plot <- function(data, y_axis, x_axis, var) {
  ...
  geom_point(data = plot_data %>% filter(id %in% retransmitted), aes(x={{x_axis}}, y={{y_axis}}, fill = id), 
             shape = 21, size = 3, colour = "transparent") +
...
# create a new function for a slimmed plot
SSF_plot <- function(data, y_axis, x_axis, error, var) {
  # select the data to plot
  plot_data = data %>% filter(variable == var)
  # plot them
  ...
  geom_point(position = position_dodge(0.5), aes(x={{x_axis}}, y={{y_axis}}, group = id_year))+
  # make a line at y = 0 for clarity
  geom_hline(yintercept = 0) +
  # remove the labels
  labs(x = "", y="") +
  scale_y_continuous(breaks = round(seq(min(plot_data$coefs), max(plot_data$coefs), by = 0.3),1))+
  # remove background grid, make axis labels black, and increase text size
  theme_classic() +
    theme(legend.position = "none", text = element_text(size=15), 
        axis.text.x = element_text(colour = "black"),
...
support_plot <- SSF_plot(SSF_results, coefs, category, var = "Wind support") + ggtitle("Wind support")
uplift_plot <- SSF_plot(SSF_results, coefs, category, var = "Boundary layer height") + ggtitle("Uplift (BLH)")
interaction_plot <- SSF_plot(SSF_results, category, Migration, var = "Interaction") + ggtitle("Wind support x Uplift")
...
# tail_SSF_results <- SSF_results %>% filter(var == "Wind support")
# # create a new variable that contains the individuals surviving multiple migrations
# tail_plot_highlight <- tail_SSF_results %>% filter(id == "Jaana" | id == "Senta" | 
#                                                  id == "Valentin" | id == "Mohammed" | 
#                                                  id == "Lars")
# # create a new variable with only individuals that died before a second migration 
# tail_SSF_results_points <- tail_SSF_results %>% filter(id != "Jaana" & id != "Senta" & 
#                                                    id != "Valentin" & id != "Mohammed" & 
#                                                    id != "Lars")
# # make the plot
# tail_plot <- ggplot(tail_SSF_results, aes(x=Migration, y=ssf_coefsTU)) + 
#   # a box plot with all the data
#   geom_boxplot(outlier.shape = NA, color = "grey30", linetype="dashed")+ 
#   # a second box plot without whiskers
#   stat_boxplot(aes(ymin = ..lower.., ymax = ..upper..), outlier.shape = NA, fill = "grey80",
#                lwd = 0.75, fatten = 1.2) +
#   # the points for individuals that died
#   geom_jitter(data = tail_SSF_results_points, aes(), size=2, width=0.3, color = "grey30") +
#   # the points for individuals with multiple migrations
#   geom_point(data = tail_plot_highlight, aes(x=Migration, y=ssf_coefsTU, fill = id), 
#              shape = 21, size = 3, colour = "transparent") +
#   # the lines connecting the repeated journeys
...
# lift_SSF_results <- SSF_results %>% filter(var == "Boundary layer height")
# lift_plot_highlight <- lift_SSF_results %>% filter(id == "Jaana" | id == "Senta" | 
#                                                  id == "Valentin" | id == "Mohammed" | 
#                                                  id == "Lars")
# lift_SSF_results_points <- lift_SSF_results %>% filter(id != "Jaana" & id != "Senta" & 
#                                                    id != "Valentin" & id != "Mohammed" & 
#                                                    id != "Lars")
# 
# lift_plot <- ggplot(lift_SSF_results, aes(x=Migration, y=ssf_coefsTU)) + 
#   geom_boxplot(outlier.shape = NA, color = "grey30", linetype="dashed")+
#   stat_boxplot(aes(ymin = ..lower.., ymax = ..upper..), outlier.shape = NA, fill = "grey80",
#                lwd = 0.75, fatten = 1.2) +
#   geom_jitter(data = lift_SSF_results_points, aes(), size=2, width=0.3, color = "grey30") +
#   geom_point(data = lift_plot_highlight, aes(x=Migration, y=ssf_coefsTU, fill = id), 
#              shape = 21, size = 3, colour = "transparent")  + 
#   geom_line(data = lift_plot_highlight, aes(group = id, color =id), size = 1.2) + 
#   scale_fill_manual(values = id_colors) + 
#   scale_color_manual(values = id_colors) + 
#   labs(title = "Uplift (BLH)", x="Migration", y = NULL ) +
#   geom_hline(yintercept = 0) +
#   theme_classic() +
#   theme(legend.position = "none", text = element_text(size=15), 
#         axis.text.x = element_text(colour = "black"),
#         axis.text.y = element_text(colour = "black"))
# 
# inter_SSF_results <- SSF_results %>% filter(var == "Interaction")
# inter_plot_highlight <- inter_SSF_results %>% filter(id == "Jaana" | id == "Senta" | 
#                                                  id == "Valentin" | id == "Mohammed" | 
#                                                  id == "Lars")
# inter_SSF_results_points <- inter_SSF_results %>% filter(id != "Jaana" & id != "Senta" & 
#                                                    id != "Valentin" & id != "Mohammed" & 
#                                                    id != "Lars")
# 
# inter_plot <- ggplot(inter_SSF_results, aes(x=Migration, y=ssf_coefsTU)) + 
#   geom_boxplot(outlier.shape = NA, color = "grey30", linetype="dashed")+
#   stat_boxplot(aes(ymin = ..lower.., ymax = ..upper..), outlier.shape = NA, fill = "grey80",
#                lwd = 0.75, fatten = 1.2) +
#   geom_jitter(data = inter_SSF_results_points, aes(), size=2, width=0.3, color = "grey30") +
#   geom_point(data = inter_plot_highlight, aes(x=Migration, y=ssf_coefsTU, fill = id), 
#              shape = 21, size = 3, colour = "transparent")  + 
#   geom_line(data = inter_plot_highlight, aes(group = id, color =id), size = 1.2) + 
#   scale_fill_manual(values = id_colors) + 
#   scale_color_manual(values = id_colors) + 
#   labs(title = "Wind support x Uplift", x="Migration", y = NULL ) +
#   geom_hline(yintercept = 0) +
#   theme_classic() +
#   theme(legend.position = "none", text = element_text(size=15), 
#         axis.text.x = element_text(colour = "black"),
...
# 
# 
# ggarrange(tail_plot, lift_plot, inter_plot, ncol = 3, legend = "none")#, sl_plot, ta_plot, ncol=3, nrow=2, legend = "none")
```


Figure 1: The importance of wind support, uplift, and their interaction to route selection over migrations. Negative values of vertical velocity correspond to rising air. We fitted separate conditional logistic regressions for each track and extracted the coefficients for each predictor. Coefficients of individuals tracked in multiple years are connected by lines and highlighted in color.
...
# pare the data down
validation_data <- annotated_data %>% select(case_, id, year, burst_, step_id_, scaled_tail, scaled_blh, scaled_step, scaled_angle)  %>%  na.omit()
...
```{r validation_model,warning=F, eval=F}
# fit the model to 80% of the data
validation_model <- clogit(case_ ~ scaled_tail + scaled_blh + scaled_step + scaled_angle + scaled_tail*scaled_blh + strata(step_id_), data = training_data)
```

```{r valid_tibble, echo=F,message=F}
load("validation_model_Feb8.RData")
```

```{r validation2,warning=F}
...
# i.e. all available permutations
no.perm <- 101 

# label the data with life stage and migration information. This is a repetition of what 
# was done above, but here we label the full data rather than the coefficients
...
annotated_data$Migration[which(annotated_data$id_year == "Jaana_2013" | # add year 2 info
                                 ...
                                 annotated_data$id_year == "Valentin_2015")] <- "second"
...
annotated_data$Migration[which(annotated_data$id_year == "Jaana_2014" | # add year 3 info
                                 ...
                                 annotated_data$id_year == "Senta_2016")] <- "third"

annotated_data$Migration[which(annotated_data$id_year == "Lars_2015" | # add year 4 info
                                 annotated_data$id_year == "Mohammed_2020")] <- "z_fourth"
...
annotated_data$Migration[which(is.na(annotated_data$Migration))] <- "first" # everything else is year 1
...
clogit_results <- annotated_data %>% 
  group_by(Migration) %>% 
  nest() %>% 
  mutate(model_col = purrr::map(data, modelTU),
         coefsTU = purrr::map(model_col, coef)) %>% 
  unnest(cols = c(coefsTU, Migration)) %>% 
  mutate(variable = c("tail","blh", "sl_", "ta_", "interaction")) %>%
  arrange(variable, Migration)
...
# row.names(random_model) <- sub("cos\\(ta_\\)", "ta_", row.names(random_model))
# row.names(random_model) <- sub("log\\(sl_\\)", "sl_", row.names(random_model))
...
for (i in sort(migrations)) {
  ...
  for (j in sort(variable)) {
    ...
    significance <- sum(abs(temp_obs$coefsTU) < abs(temp_rand$Estimate))/no.perm
...
Plot the used and available wind support and uplift.

```{r supplots}
# The values of wind support (m/s) and uplift (Pa/s) available and used in each migratory
# journey

# a function for figure legends
add_legend <- function(...) {
...
labels <- c("Wind support (m/s)", "Vertical velocity (Pa/s)")
variables <- c("tail", "vertical_pressure")
...
# loop through each box and plots it
for(i in 1:length(variables)){
   # an empty plot
  boxplot(unlist(annotated_data[,variables[i]]) ~ unlist(annotated_data[,"Migration"]), 
          data = annotated_data, boxfill = NA, border = NA, main = " ", 
          xlab = "Migration", ylab = labels[i])
   # boxplots for the used points 
  boxplot(unlist(annotated_data[annotated_data$case_ == 1, variables[i]]) ~
            unlist(annotated_data[annotated_data$case_ == 1,"Migration"]),
          xlab = "Migration", ylab = labels[i],
          yaxt = "n", xaxt = "n", add = T, boxfill = "cornflowerblue", 
          boxwex = 0.25, at = c(1,2,3,4,5) - 0.15)
  # boxplots for available points
  boxplot(unlist(annotated_data[annotated_data$case_ == 0, variables[i]]) ~
            unlist(annotated_data[annotated_data$case_ == 0, "Migration"]),
          yaxt = "n", xaxt = "n", add = T, boxfill = "grey", 
          boxwex = 0.25, at = c(1,2,3,4,5) + 0.15)
  
}

# add the legend with the function above
add_legend("bottom", legend=c("Used", "Available"), pch=20, 
           col=c("cornflowerblue", "gray"),
           horiz=TRUE, bty='n', cex=1.3, pt.cex = 2)

# The values of wind support (m/s) and uplift (Pa/s) available and used in each calendar
# year of collection

years <- sort(unique(annotated_data$year))

# reset the plotting window
par(mfrow = c(1, 2), 
    oma = c(3,0,0,0),  #outer margin
    las = 1)

# loop through each category and plot
for(i in 1:length(variables)){
  
  boxplot(unlist(annotated_data[,variables[i]]) ~ unlist(annotated_data[,"year"]), 
          data = annotated_data, boxfill = NA, border = NA, main = " ", xlab = "Year",
          ylab = labels[i])
  boxplot(unlist(annotated_data[annotated_data$case_ == 1, variables[i]]) ~
            unlist(annotated_data[annotated_data$case_ == 1,"year"]), 
          yaxt = "n", xaxt = "n", add = T, boxfill = "cornflowerblue", 
          boxwex = 0.25, at = c(1,2,3,4,5,6,7,8,9) - 0.15)
  axis(1, at=1:9, labels=years[1:9])
  boxplot(unlist(annotated_data[annotated_data$case_ == 0, variables[i]]) ~
            unlist(annotated_data[annotated_data$case_ == 0, "year"]),
          yaxt = "n", xaxt = "n", add = T, boxfill = "grey",
          boxwex = 0.25, at = c(1,2,3,4,5,6,7,8,9) + 0.15)
...
Finally, plot the distributions of coefficients derived from randomized data along with the observed coefficients.
...
trip <- sort(migrations)

# write a function to draw the plots
null_plot <- function(data, x_axis, journey, var) {
  # select the variable to plot
  var = rownames(data)[1]
  # set the name to remove "scaled_" as needed
  var <- ifelse(grepl("scaled_", var) == T, sub("scaled_", "", var), var)
  ...
  ggplot(data %>% filter(Migration == {{journey}}), aes({{x_axis}})) + 
    ...
    geom_vline(aes(xintercept = clogit_results[which(clogit_results$Migration == {{journey}} & grepl({{var}}, clogit_results$variable) == T),]$coefsTU), 
               color="red", size=1)+ 
    theme_classic() + 
    theme(legend.position = "none", text = element_text(size=15), 
...
ct <- filter(random_model, grepl("tail", rownames(random_model)))
cl <- filter(random_model, grepl("blh", rownames(random_model)))
ci <- filter(random_model, grepl("interaction", rownames(random_model)))
cs <- filter(random_model, grepl("sl_", rownames(random_model)))
ca <- filter(random_model, grepl("ta_", rownames(random_model)))
...
first_step <- null_plot(cs, Estimate, journey = trip[1]) + ggtitle("log Step length")
second_step <- null_plot(cs, Estimate, journey = trip[2]) + ggtitle(" ")
third_step <- null_plot(cs, Estimate, journey = trip[3]) + ggtitle(" ")
fourth_step <- null_plot(cs, Estimate, journey = trip[4]) + ggtitle(" ")
fifth_step <- null_plot(cs, Estimate, journey = trip[5]) + ggtitle(" ")

first_angle <- null_plot(ca, Estimate, journey = trip[1]) + ggtitle("cos Turning angle")
second_angle <- null_plot(ca, Estimate, journey = trip[2]) + ggtitle(" ")
third_angle <- null_plot(ca, Estimate, journey = trip[3]) + ggtitle(" ")
fourth_angle <- null_plot(ca, Estimate, journey = trip[4]) + ggtitle(" ")
fifth_angle <- null_plot(ca, Estimate, journey = trip[5]) + ggtitle(" ")
...
all_15 <- ggarrange(first_tail,second_tail,third_tail,fourth_tail,fifth_tail, 
          ...
          # first_angle,second_angle,third_angle,fourth_angle,fifth_angle,
          # first_step,second_step,third_step,fourth_step,fifth_step,
          ncol = 5, nrow=3)

# add common axes
annotate_figure(all_15, left = text_grob("Frequency", rot = 90, size = 20),
