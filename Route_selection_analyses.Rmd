...
modelTCV <- function(df) {
  clogit(case_ ~ scaled_tail + scaled_cross + scaled_vertical + strata(step_id_), data = df)
}
...
# disregard data from earlier migrations by adults of unknown age
annotated_data <- annotated_data[which(annotated_data$stage == "juvenile" |
                                       annotated_data$stage == "adult" &
                                       annotated_data$Migration == "5 +"),]
...
first_tail <- ggplot(data = filter(random_model, 
  ...
  geom_histogram(fill="black", bins = no.bin) + ggtitle("Wind support") +
...
first_cross <- ggplot(data= filter(random_model, 
                                ...
                                & Migration == 1), aes(Estimate)) + 
  geom_histogram(fill="black", bins = no.bin) + ggtitle("Crosswind") +
...
first_lift <- ggplot(data= filter(random_model, 
                                ...
                                & Migration == 1), aes(Estimate)) + 
  geom_histogram(fill="black", bins = no.bin) + ggtitle("Vertical velocity") +
  geom_vline(aes(xintercept = filter(clogit_results, variable == "vertical" & 
                               Migration == 1)$coefsTCV), color="red", size=1) +
  theme_classic()+
    theme(legend.position = "none", text = element_text(size=15), 
          axis.text = element_text(colour = "black"), 
          axis.title.y = element_blank()) + labs(x = "1")

second_tail <- ggplot(data= filter(random_model, 
                               grepl("tail", rownames(random_model)) & 
                               Migration == 2), aes(Estimate)) + 
  geom_histogram(fill="black", bins = no.bin) + ggtitle(" ") +
  geom_vline(aes(xintercept = filter(clogit_results, variable == "tail" & 
...
second_lift <- ggplot(data= filter(random_model, 
  ...
  theme_classic()+
    theme(legend.position = "none", text = element_text(size=15), 
          axis.text = element_text(colour = "black"), 
          axis.title.y = element_blank()) + labs(x = "2")
...
third_lift <- ggplot(data= filter(random_model, 
  ...
  theme_classic()+
    theme(legend.position = "none", text = element_text(size=15), 
          axis.text = element_text(colour = "black"), 
          axis.title.y = element_blank()) + labs(x = "3")
...
fourth_lift <- ggplot(data= filter(random_model, 
  ...
  theme_classic()+
    theme(legend.position = "none", text = element_text(size=15), 
          axis.text = element_text(colour = "black"), 
          axis.title.y = element_blank()) + labs(x = "4")
...
adult_lift <- ggplot(data= filter(random_model, 
    ...
    theme(legend.position = "none", text = element_text(size=15), 
          axis.text = element_text(colour = "black"), 
          axis.title.y = element_blank()) + labs(x = "5 +")

all_15 <- ggarrange(first_tail,second_tail,third_tail,fourth_tail,adult_tail, 
                    first_cross,second_cross,third_cross,fourth_cross,adult_cross, 
                    first_lift,second_lift,third_lift,fourth_lift,adult_lift, 
                    ncol = 5, nrow=3)
