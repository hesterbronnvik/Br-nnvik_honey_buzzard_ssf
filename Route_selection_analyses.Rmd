...
SSF_results$category[which(SSF_results$Migration %in% c(2,3,4))] <- "Intermediate"
SSF_results$category <- factor(SSF_results$category, levels = c("First year", "Intermediate", "Adult"))
...
SSF_plot <- function(data, y_axis, x_axis, error, var) {
  ...
  geom_boxplot(outlier.shape = NA, color = "slategray3", linetype="dashed", lwd = 1)+ 
  # a second box plot without whiskers
  stat_boxplot(aes(ymin = ..lower.., ymax = ..upper..), outlier.shape = NA, fill = "#DBE3F3", color = "slategray3", lwd = 1, fatten = 1) +
    geom_errorbar(
    aes(ymin = coefs-se, ymax = coefs+se, group = id_year),
    position = position_dodge(0.8), width = 1, color = "grey50"
    )+
  geom_point(position = position_dodge(0.8), aes(x={{x_axis}}, y={{y_axis}}, group = id_year), size = 2)+
  # make a line at y = 0 for clarity
  geom_hline(yintercept = 0) +
  # remove the labels
  labs(x = "", y="") +
  #scale_y_continuous(breaks = round(seq(min(plot_data$coefs), max(plot_data$coefs), by = 0.5),1))+
...
interaction_plot <- SSF_plot(SSF_results, coefs, category, var = "Interaction") + ggtitle("Wind support x Uplift")
...
# create the plots
first_tail <- null_plot(ct, Estimate, journey = trip[2]) + ggtitle("Wind support")
second_tail <- null_plot(ct, Estimate, journey = trip[3]) + ggtitle(" ")
third_tail <- null_plot(ct, Estimate, journey = trip[4]) + ggtitle(" ")
fourth_tail <- null_plot(ct, Estimate, journey = trip[5]) + ggtitle(" ")
fifth_tail <- null_plot(ct, Estimate, journey = trip[1]) + ggtitle(" ")

first_lift <- null_plot(cl, Estimate, journey = trip[2]) + ggtitle("Uplift (BLH)")
second_lift <- null_plot(cl, Estimate, journey = trip[3]) + ggtitle(" ")
third_lift <- null_plot(cl, Estimate, journey = trip[4]) + ggtitle(" ")
fourth_lift <- null_plot(cl, Estimate, journey = trip[5]) + ggtitle(" ")
fifth_lift <- null_plot(cl, Estimate, journey = trip[1]) + ggtitle(" ")

first_inter <- null_plot(ci, Estimate, journey = trip[2]) + ggtitle("Wind x Uplift")
second_inter <- null_plot(ci, Estimate, journey = trip[3]) + ggtitle(" ")
third_inter <- null_plot(ci, Estimate, journey = trip[4]) + ggtitle(" ")
fourth_inter <- null_plot(ci, Estimate, journey = trip[5]) + ggtitle(" ")
fifth_inter <- null_plot(ci, Estimate, journey = trip[1]) + ggtitle(" ")

first_step <- null_plot(cs, Estimate, journey = trip[2]) + ggtitle("log Step length")
second_step <- null_plot(cs, Estimate, journey = trip[3]) + ggtitle(" ")
third_step <- null_plot(cs, Estimate, journey = trip[4]) + ggtitle(" ")
fourth_step <- null_plot(cs, Estimate, journey = trip[5]) + ggtitle(" ")
fifth_step <- null_plot(cs, Estimate, journey = trip[1]) + ggtitle(" ")

first_angle <- null_plot(ca, Estimate, journey = trip[2]) + ggtitle("cos Turning angle")
second_angle <- null_plot(ca, Estimate, journey = trip[3]) + ggtitle(" ")
third_angle <- null_plot(ca, Estimate, journey = trip[4]) + ggtitle(" ")
fourth_angle <- null_plot(ca, Estimate, journey = trip[5]) + ggtitle(" ")
fifth_angle <- null_plot(ca, Estimate, journey = trip[1]) + ggtitle(" ")
